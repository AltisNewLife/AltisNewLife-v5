...
*/
private ["_shop","_priceTag","_price","_item","_itemArray","_bool"];
params [
...
if ((uiNamespace getVariable ["Weapon_Shop_Filter",0]) isEqualTo 1) then {
    ...
    _item = [_item,_itemArray] call life_fnc_getIndex;
    ...
    _item = CONTROL_DATAI(_control,_index);
    if (_price > CASH) then {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Prix : <t color='#ff0000'>%1 €</t><br/>Il vous manque : <t color='#8cff9b'>%2 €</t></t>",[(_price)] call life_fnc_numberText,[(_price - CASH)] call life_fnc_numberText];
    } else {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Prix : <t color='#8cff9b'>%1 €</t></t>",[(_price)] call life_fnc_numberText];
    };
